<?php
// phpBB login integration ( script que pode ser usado para criar  fóruns)
define ("PHPBB","0");
...
define ("TITLE", "WebTester Online Teste");
...
define ("CLOSE_WINDOW", "1");    // Closes window when 'Done' is pressed
define ("RETRY", "0");           // Places Retry button on grade page
define ("NODONE", "0");          // Removes Done button on grade page
define ("ALLOW_OVERRIDE", "1");  // Disables strict session checking
...
if((function_exists("get_magic_quotes_gpc") && get_magic_quotes_gpc())    || (ini_get('magic_quotes_sybase') && (strtolower(ini_get('magic_quotes_sybase'))!="off")) ) {
 ...
 $HTTP_SERVER_VARS = stripslashes_deep($HTTP_SERVER_VARS);
 $HTTP_GET_VARS = stripslashes_deep($HTTP_GET_VARS);
 $HTTP_POST_VARS = stripslashes_deep($HTTP_POST_VARS);
 $HTTP_COOKIE_VARS = stripslashes_deep($HTTP_COOKIE_VARS);
 $HTTP_POST_FILES = stripslashes_deep($HTTP_POST_FILES);
 $HTTP_ENV_VARS = stripslashes_deep($HTTP_ENV_VARS);
 if (isset($_SESSION)) {  #These are unconfirmed (?)
   $_SESSION = stripslashes_deep($_SESSION, '');
   $HTTP_SESSION_VARS = stripslashes_deep($HTTP_SESSION_VARS, '');
 }
 /*
