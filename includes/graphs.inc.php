...
/*
 +-------------------------------------------------------------------+
 |                   H T M L - G R A P H S   (v4.10)                 |
 |                                                                   |
 | Copyright Gerd Tentler                www.gerd-tentler.de/tools   |
 | Created: Sep. 17, 2002                Last modified: Jan. 7, 2012 |
 +-------------------------------------------------------------------+
...
//----------------------------------------------------------------------------------------------------
	...
	var $legendAbsValues = false;              // view abs. values in legend: true = yes, false = no
...
//----------------------------------------------------------------------------------------------------
	...
	function build_legend($barColors, $barValues) {
		...
		for($i = 0; $i < count($barColors); $i++) {
			...
			if($this->legendAbsValues) {
				for($chart = $sum = 0; $chart < count($barValues); $chart++) {
					foreach($barValues[$chart] as $label) $sum += $label[$i];
				}
				$legend .= '<td style="' . $this->cssLEGEND . '" nowrap>(' . $this->absValuesPrefix . $sum . $this->absValuesSuffix . ')</td>';
			}
			$legend .= '</tr>';
		}
	...
	function create() {
		...
		for($i = $sum = $max = $max_neg = $max_dec = $ccnt = $lcnt = $chart = 0; $i < $bars; $i++) {
			...
			for($j = $dec = 0; $j < count($drv); $j++) {
				...
				if($this->type == 'hbar' || $this->type == 'vbar' || $j == 0) {
					if($v > $max) $max = $v;
