...
var TinyMCE_AdvancedTheme = {
	...
	getControlHTML : function(button_name) {
		...
		switch (button_name) {
			case "formatselect":
				...
				var formats = tinyMCE.getParam("theme_advanced_blockformats", "p,address,pre,h1,h2,h3,h4,h5,h6", true).explode(',');
				var lookup = [
			...
			case "fontselect":
				...
				var fonts = tinyMCE.getParam("theme_advanced_fonts", nFonts).explode(';');
				for (i=0; i<fonts.length; i++) {
					if (fonts[i] != '') {
						var parts = fonts[i].explode('=');
	...
	getEditorTemplate : function(settings, editorId) {
		...
		if (settings['theme_advanced_styles']) {
			var stylesAr = settings['theme_advanced_styles'].explode(';');
			
			for (var i=0; i<stylesAr.length; i++) {
				var key, value;

				key = stylesAr[i].explode('=')[0];
				value = stylesAr[i].explode('=')[1];
