...
if ($phpThumb->config_mysqli_query) {
	if ($cid = @mysqli_connect($phpThumb->config_mysqli_hostname, $phpThumb->config_mysqli_username, $phpThumb->config_mysqli_password)) {
		if (@mysqli_select_db($phpThumb->config_mysqli_database, $cid)) {
			if ($result = @mysqli_query($phpThumb->config_mysqli_query, $cid)) {
				...
				mysqli_close($cid);
				$phpThumb->ErrorImage('Error in mysqli query: "'.mysqli_error($cid).'"');
			}
		} else {
			mysqli_close($cid);
			$phpThumb->ErrorImage('cannot select mysqli database: "'.mysqli_error($cid).'"');
		}
	} else {
		$phpThumb->ErrorImage('cannot connect to mysqli server');
	}
	unset($_GET['id']);
}
