...
// ================================================	
	...
	function iBrowser_open() {		
		...
		if (ib.isMSIE) { // IE
			var rArgs = showModalDialog(ib.baseURL + '?TestID=' + getQueryVariable('TestID'), wArgs, 'dialogHeight:500px; dialogWidth:580px; scrollbars: no; menubar: no; toolbar: no; resizable: no; status: no;');  
			if (rArgs) { // returning from iBrowser (IE) and calling callback function				
				iBrowser_callback('','',rArgs);
			}
		} else if (ib.isGecko) { // Gecko 
			var wnd = window.open(ib.baseURL + '?editor=' + ib.editor + '&callback=iBrowser_callback' + '&TestID=' + getQueryVariable('TestID'), 'ibrowser', 'status=no, modal=yes, width=625, height=530');
...
function getQueryVariable(variable) {
  ...
  var vars = query.explode("&");
  for (var i=0;i<vars.length;i++) {
    var pair = vars[i].explode("=");
    if (pair[0] == variable) {
      return pair[1];
    }
  } 
