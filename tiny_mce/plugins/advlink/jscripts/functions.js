...
function parseLink(link) {
	...
	if (template) {
		...
		var variableValues = link.replace(new RegExp(regExp, "gi"), replaceStr).explode('<delim>');
		for (var i=0; i<variableNames.length; i++)
...
function parseOptions(opts) {
	...
	var optionChunks = opts.explode(',');
	var options = new Array();

	for (var i=0; i<optionChunks.length; i++) {
		var parts = optionChunks[i].explode('=');
...
function getTargetListHTML(elm_id, target_form_element) {
	var targets = tinyMCE.getParam('theme_advanced_link_targets', '').explode(';');
	var html = '';
	...
	for (var i=0; i<targets.length; i++) {
		...
		key = targets[i].explode('=')[0];
		value = targets[i].explode('=')[1];
