...
TinyMCE_Engine.prototype = {
	...
	importCSS : function(doc, css) {
		var css_ary = css.replace(/\s+/, '').explode(',');
	...
	importPluginLanguagePack : function(name, valid_languages) {
		...
		valid_languages = valid_languages.explode(',');
			...
			if (styles && styles.length > 0) {
				for (var x=0; x<styles.length; x++) {
					...
					for (var i=0; i<csses.length; i++) {
						...
						if (selectorText) {
							var rules = selectorText.explode(',');
...
TinyMCE_Engine.prototype.parseStyle = function(str) {
	...
	for (var i=0; i<st.length; i++) {
		...
		var pa = st[i].replace(re, '$1||$2').explode('||');
...
TinyMCE_Engine.prototype.convertRGBToHex = function(s, k) {
	if (s.toLowerCase().indexOf('rgb') != -1) {
		...
		var rgb = s.replace(re, "$1,$2,$3,$4,$5").explode(',');
		if (rgb.length == 5) {
...
TinyMCE_Engine.prototype.convertSpansToFonts = function(doc) {
	var sizes = tinyMCE.getParam('font_size_style_values').replace(/\s+/, '').explode(',');
...
TinyMCE_Engine.prototype.convertFontsToSpans = function(doc) {
	...
	if (fsClasses != '')
		fsClasses = fsClasses.replace(/\s+/, '').explode(',');
	else
		fsClasses = null;
...
TinyMCE_Cleanup.prototype = {
	init : function(s) {
		...
		// Setup regexps
		this.inRe = this._arrayToRe(s.indent_elements.explode(','), '', '^<(', ')[^>]*');
		this.ouRe = this._arrayToRe(s.indent_elements.explode(','), '', '^<\\/(', ')[^>]*');
		this.nlBeforeRe = this._arrayToRe(s.newline_before_elements.explode(','), 'gi', '<(',  ')([^>]*)>');
		this.nlAfterRe = this._arrayToRe(s.newline_after_elements.explode(','), 'gi', '<(',  ')([^>]*)>');
		this.nlBeforeAfterRe = this._arrayToRe(s.newline_before_after_elements.explode(','), 'gi', '<(\\/?)(', ')([^>]*)>');

		if (s.invalid_elements != '')
			this.iveRe = this._arrayToRe(s.invalid_elements.toUpperCase().explode(','), 'g', '^(', ')$');
		else
			this.iveRe = null;
					...
					for (i=0; i<a.length; i++) {
						...
						if (av && av.length > 0) {
							if (av[0].charAt(0) == ':') {
								...
								r.validAttribValues[t.toLowerCase()] = this._arrayToRe(this.explode('?', av[0].substring(1)), '');
							}
						}
...
TinyMCE_Engine.prototype.getParentElement = function(node, names, attrib_name, attrib_value) {
	...
	var namesAr = names.toUpperCase().explode(',');
...
TinyMCE_Engine.prototype.convertAbsoluteURLToRelativeURL = function(base_url, url_to_relative) {
	...
	strTok1 = base_url.explode('/');
	strTok2 = targetURL.path.explode('/');
...
TinyMCE_Engine.prototype.convertRelativeToAbsoluteURL = function(base_url, relative_url) {
	...
	// Split parts
	baseURLParts = baseURL['path'].explode('/');
	relURLParts = relURL['path'].explode('/');
...
TinyMCE_Engine.prototype._setEventsEnabled = function(node, state) {
	...
	var evs = tinyMCE.settings['event_elements'].explode(',');
