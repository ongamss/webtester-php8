...
?>
		   ...
		   <!-- InstanceBeginEditable name="nav" --><a href="index.php">WebTester</a> &gt; <a href="./testManage.php">Test Management</a> &gt; Delete Tests <!-- InstanceEndEditable --></div></td>
    </tr>
    <tr> 
      <td align="center" valign="top">		<div align="left"><!-- InstanceBeginEditable name="Content Area" -->
		...
		if (isset($_POST['Dup_x'])) {
			if (count($_POST['delete']) > 1) {
				redirect_to("dupTooMany.php");
			} else {
				redirect_to("dup.php?testID=" . $_POST['delete'][0]);
			}
		}
		if (isset($_POST['No'])) {
			redirect_to("testManage.php");
		...
		if (isset($_POST['confirm']) && $_POST['confirm']==1) {
			...
			for ($i = 0; $i < count($_POST['delete']); $i++) {
				// delete test from db
				$testsSQL="DELETE FROM tests  WHERE ID=" . $_POST['delete'][$i];
				$myRs=mysqli_query($conn, $testsSQL)
					or die("Invalid Query: " . $testsSQL . " - " . mysqli_error());
				// delete questions from db
				$questionsSQL="DELETE FROM questions WHERE TestID='" . $_POST['delete'][$i] . "'";
				$myRs=mysqli_query($conn, $questionsSQL)
					or die("Invalid Query: " . $questionsSQL . " - " . mysqli_error());
				// delete images
				$folderToDelete=realpath("../test-images/" . $_POST['delete'][$i]);
				if (file_exists($folderToDelete)) rm($folderToDelete);
			}
			redirect_to("testManage.php");
		}
		?>
		<span class="style7">Delete Tests</span></p>
		<p>Are you sure you want to delete the following tests?</p>
  ...
  <tr bgcolor="#C8D8FF">
    ...
    <td align="left" class="style1 style5">Name</td>
  </tr>
 
          <?php
	for ($i = 0; $i < count($_POST['delete']); $i++) {
		$testsSQL="SELECT * FROM tests  WHERE ID=" . $_POST['delete'][$i];
		...
		<td width="100" class="style1"><input name="delete[]" type="hidden" value="<?=$myRs['ID']?>"><?=$myRs['ID']?></td>
		<td class="style1"><?=$myRs['TestName']?></td>
		<?php
		// echo("<td width='100'><input name='delete[]' type='hidden' value='" . $myRs['ID'] . "'>" . $myRs['TestName'] . " - ID#");
	  	// echo($_POST['delete'][$i]);
		?>
		</tr>
		<?php
	  ...
	  ?>
	      ...
	      <p>
	        <input name='confirm' type='hidden' value='1'>
	          <input name='Yes' type='submit' id='Del' value='Yes'>
