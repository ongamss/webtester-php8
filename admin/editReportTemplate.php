  ...
  <table width="100%" height="50" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#333333" BORDERCOLORLIGHT="#999999" BORDERCOLORDARK="#333333">
    <tr> 
      ...
      <td align="center" valign="middle"><!-- InstanceBeginEditable name="CurrentArea" -->
        <p class="style4">Edit Report Template </p>
...
?>
		   ...
		   <!-- InstanceBeginEditable name="nav" --><a href="index.php">WebTester</a> &gt; <a href="./preferences.php">Preferences</a> &gt; <a href="./reportTemplates.php">Report Templates</a> &gt; Edit Report Template <!-- InstanceEndEditable --></div></td>
    </tr>
    <tr> 
	  ...
	  $emailSQL="SELECT * FROM reporttemplates WHERE ID=" . $_REQUEST['ID'];
	  $myRsRes=mysqli_query($conn, $emailSQL);
	  $row=mysqli_fetch_assoc($myRsRes);
		?>
		<form name="form1" method="post" action="updateReportTemplate.php">
		<p class="style7">Edit Report Template </p>
  ...
  <tr class="d0" onMouseOver="javascript:highlightTableRowVersionA(this, '#FFFF99');">
    <td width="125" valign="top" class="style1">Content:<div id="safari" style="visibility:hidden"> (HTML editor not supported in Safari)</div></td>
    <td><textarea name="text" cols="80" rows="15" id="text"><?=$row['Text']?><?php
					...
					if(isset($_POST["text"]))
					{
						$sContent=stripslashes($_POST['text']); //Remove slashes
  ...
  <?php
  $pointSQL            = "SELECT * FROM custommessages WHERE ReportID=" . $_REQUEST['ID'];
  $pointSQLRes         = mysqli_query($conn, $pointSQL);
  $num_custom_messages = mysqli_num_rows($pointSQLRes);
  ?>
  <tr class="d1" onMouseOver="javascript:highlightTableRowVersionA(this, '#FFFF99');">
    <td valign="top" class="style1">Custom Messages:</td>
    <td><span class="style1"><a href="./editCustomMessages.php?reportID=<?=$_REQUEST['ID']?>">
      <?=$num_custom_messages?></a> (Must use %CUSTOMMESSAGE% tag to show)</span></td>
  </tr>
  <tr align="center">
