  ...
  <table width="100%" height="50" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#333333" BORDERCOLORLIGHT="#999999" BORDERCOLORDARK="#333333">
    <tr> 
      ...
      <td align="center" valign="middle"><!-- InstanceBeginEditable name="CurrentArea" -->
        <p class="style4">Delete Questions </p>
...
?>
		  ...
		  ?>
		  <a href="index.php">WebTester</a> &gt; <a href="./testManage.php">Test Management</a> &gt; <a href="./editTest.php?TestID=<?=$testid?>">Edit Test</a> &gt; <a href="./questionEdit.php?TestID=<?=$testid?>">Question Editor</a> &gt; Delete Questions <!-- InstanceEndEditable --></div></td>
    </tr>
    <tr> 
	  ...
	  <?php
		...
		if (isset($_POST['confirm'])) {
			$del=$_POST['delete'];
			foreach ($del as $d) {
				$questionsSQL="DELETE FROM questions WHERE ID='" . $d . "'";
				echo("Removing Question ID: " . $d . "<br>");
		...
		?>
          <p class="style7">Delete Questions </p>
          <p>Are you sure you want to delete the following questions?
            </p>
<table width="100%"  border="0" cellspacing="2" cellpadding="0" onMouseOut="javascript:highlightTableRowVersionA(0);">
  <tr bgcolor="#C8D8FF">
    ...
    <td align="left" class="style1 style5">Question</td>
  </tr>
          <?php
	  echo("<form action='deleteQuestion.php' method='post' name='delete' id='delete'>");
	  $del=$_POST['delete'];
	  if (isset($_REQUEST['quesID'])) {
			...
			echo("<td align='left' class='style1 style5'>" . $row['QuestionText'] . "</td>");
			?>
			</tr>
			<?php
	  } else {
	  		foreach ($del as $d) {
				$questionsSQL="SELECT * FROM questions WHERE ID=" . $d;
				$myRsRes=mysqli_query($conn, $questionsSQL);
				$row=mysqli_fetch_assoc($myRsRes);
				$i++;
				?>
	...
	if (isset($_REQUEST['TestID'])) {
		echo("<input name='TestID' type='hidden' value='" . $_REQUEST['TestID'] . "'>");
	} else {
		echo("<input name='TestID' type='hidden' value='" . $_POST['TestID'] . "'>");
	}
	?>
	<div align="center">
	<?php
