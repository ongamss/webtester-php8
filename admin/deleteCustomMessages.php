  ...
  <table width="100%" height="50" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#333333" BORDERCOLORLIGHT="#999999" BORDERCOLORDARK="#333333">
    <tr> 
      ...
      <td align="center" valign="middle"><!-- InstanceBeginEditable name="CurrentArea" -->
        <p class="style4">Delete Custom Messages</p>
...
?>
		  ...
		  ?>
		  <a href="index.php">WebTester</a> &gt; <a href="./preferences.php">Preferences</a> &gt; <a href="./reportTemplates.php">Report Templates</a> &gt; <a href="./editReportTemplate.php?ID=<?=$_REQUEST['reportID']?>">Edit Report Template</a> &gt; Delete Custom Messages<!-- InstanceEndEditable --></div></td>
    </tr>
    <tr> 
	  ...
	  <?php
	  	...
	  	// die($_POST['delete'] . $_POST['No'] . $_POST['confirm']);
		if ($_POST['delete'] == "") {
			redirect_to("editCustomMessages.php?reportID=" . $_POST['reportID']);
			exit;
		}
		if ($_POST['No'] == "No") {
			redirect_to("editCustomMessages.php?reportID=" . $_POST['reportID']);
		...
		if (isset($_POST['confirm'])) {
			$del=$_POST['delete'];
			foreach ($del as $d) {
				$messageSQL="DELETE FROM custommessages WHERE ID='" . $d . "'";
				echo("Removing ID: " . $d . "<br>");
				$result=mysqli_query($messageSQL)
					or die("Invalid Query: " . $messageSQL . " - " . mysqli_error());
			}
			redirect_to("editCustomMessages.php?reportID=" . $_POST['reportID']);
		}
		?>
          <p class="style7">Delete Custom Messages</p>
          <p>Are you sure you want to delete the following messages?
            </p>
<table width="100%"  border="0" cellspacing="2" cellpadding="0" onMouseOut="javascript:highlightTableRowVersionA(0);">
  <tr bgcolor="#C8D8FF">
    <td align="center" class="style1 style5" width="100">ID</td>
    <td align="left" class="style1 style5">Message</td>
  </tr>
          <?php
	  echo("<form action='deleteCustomMessages.php' method='post' name='delete' id='delete'>");
	  $del=$_POST['delete'];
	  foreach ($del as $d) {
				$messageSQL="SELECT * FROM custommessages WHERE ID=" . $d;
				...
				<?php
				echo("<td align='center' class='style1 style5' width='100'><input name='delete[]' type='hidden' value='" . $row['ID'] . "'>" . $row['ID'] . "</td>");
				echo("<td align='left' class='style1 style5'>" . $row['Message'] . "</td>");
				?>
				</tr>
				<?php
	  	}
	...
	echo("<input name='reportID' type='hidden' value='" . $_REQUEST['reportID'] . "'>");
	?>
	<div align="center">
	<?php
