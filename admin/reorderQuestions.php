  ...
  <table width="100%" height="50" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#333333" BORDERCOLORLIGHT="#999999" BORDERCOLORDARK="#333333">
    <tr> 
      ...
      <td align="center" valign="middle"><!-- InstanceBeginEditable name="CurrentArea" -->
        <p class="style4">Reorder Questions </p>
...
?>
		   ...
		   <!-- InstanceBeginEditable name="nav" --><a href="index.php">WebTester</a> &gt; <a href="./testManage.php">Test Management</a> &gt; <a href="./editTest.php?TestID=<?=$_REQUEST['TestID']?>">Edit Test</a> &gt; <a href="questionEdit.php?TestID=<?=$_REQUEST['TestID']?>">Question Editor</a> &gt; Reorder Questions <!-- InstanceEndEditable --></div></td>
    </tr>
    <tr> 
      <td align="center" valign="top">		<div align="left"><!-- InstanceBeginEditable name="Content Area" --> 
		...
		<p><span class="style7">Reorder Questions</span></p>
        ...
        <table width="100%"  border="0" cellspacing="2" cellpadding="0">
          <tr bgcolor="#C8D8FF">
            ...
            <td class="style1 style5">
              <?=$num_rows?> Questions (Tip: Drag the questions around to change order) </td>
            </tr>
        </table>
	    <ul id="questions_list" class="questions_blue">
        <?php
		if ($num_rows != 0) {
		$i=1;
		...
		?>
<li id="question_<?=$question_id?>"><?=$i . "."?> <?=strip_tags($questiontext)?></li>
			<?php
			$i++;
			}
			?>
        ...
        <script type="text/javascript">
            function updateOrder()
            {
                ...
                var options = {
                                method : 'post',
                                parameters : Sortable.serialize('questions_list'),
								onComplete: function(request) {
									change('questions_list', 'questions_blue');
									window.location.reload( true );
									}
                              };
 
                new Ajax.Request('processor.php', options);
				change('questions_list', 'questions_red');
            }
 
            Sortable.create('questions_list', { onUpdate : updateOrder });
        </script>
		  <?php
		}
		?>
