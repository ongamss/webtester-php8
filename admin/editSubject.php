...
?>
		   ...
		   <!-- InstanceBeginEditable name="nav" --><a href="index.php">WebTester</a> &gt; <a href="./subjects.php">Subject Management</a> &gt; Edit Subject <!-- InstanceEndEditable --></div></td>
    </tr>
    <tr> 
	  ...
	  if ($_REQUEST['SubjectID']==1) $disabled="disabled";
	  $subjectSQL="SELECT * FROM subjects WHERE ID=" . $_REQUEST['SubjectID'];
	  $myRsRes=mysqli_query($conn, $subjectSQL);
	  $row=mysqli_fetch_assoc($myRsRes);
		?>
		<form name="form1" method="post" action="updateSubject.php">
		<p class="style7">Edit Subject</p>
  ...
  <tr class="d0" onMouseOver="javascript:highlightTableRowVersionA(this, '#FFFF99');">
    <td width="165"><span class="style1">Name:</span></td>
    <td width="150"><input name="name" type="text" id="name" value="<?=$row['Name']?>" size="60" <?=$disabled?>></td>
  </tr>
  <tr class="d1" onMouseOver="javascript:highlightTableRowVersionA(this, '#FFFF99');">
    <td width="165" valign="top" class="style1">Description:</td>
    <td><textarea name="description" cols="80" rows="15" id="description" <?=$disabled?>><?=$row['Description']?></textarea></td>
  </tr>
  <tr align="center">
    <td colspan="2" valign="top"><input name="SubjectID" type="hidden" id="SubjectID" value="<?=$_REQUEST['SubjectID']?>">
