...
?>
    ...
    <tr> 
      <td align="center" valign="top">		<div align="left"><!-- InstanceBeginEditable name="Content Area" --> 
        <p>Which test would you like to remove images or movies from?</p>
  		<?php
		$_SESSION['redirectTo'] = "";
		$testsSQL="SELECT * FROM tests  ORDER by TestName";
		$result=mysqli_query($testsSQL, $conn)
			or die("Invalid Query: " . $testsSQL . " - " . mysqli_error());
		$num_rows=mysqli_num_rows($result);
		if ($num_rows != 0) {
		?>
        <form action="remove2.php" method="get" name="form" id="form">
          <p>
            <select name="uploadFolder" id="uploadFolder">
              <?php
			while($myRs=mysqli_fetch_assoc($result)) {
			?>
              <option value="<?=$myRs['ID']?>"><?=$myRs['TestName']?></option>
			<?php
			}
			?>
            </select>
