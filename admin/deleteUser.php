...
?>
    ...
    <tr> 
      <td align="center" valign="top">		<div align="left"><!-- InstanceBeginEditable name="Content Area" -->
		...
		if (count($_POST['delete']) < 1) redirect_to("accountManage.php");
		if ($_POST['No'] == "No") {
		...
		if ($_POST['confirm'] == "1") {
			for ($i = 0; $i < count($_POST['delete']); $i++) {
				$usersSQL="DELETE FROM users WHERE ID=" . $_POST['delete'][$i];
		...
		?>
          Are you sure you want to delete the following users?</p>
        <p> 
          <?php
	  echo("<form action='deleteUser.php' method='post' name='delete' id='delete'>");
	for ($i = 0; $i < count($_POST['delete']); $i++) {
		$usersSQL="SELECT * FROM users WHERE ID=" . $_POST['delete'][$i];
		$result=mysqli_query($conn, $usersSQL);
		$myRs=mysqli_fetch_assoc($result);
		echo("<input name='delete[]' type='hidden' value='" . $myRs['ID'] . "'>" . $myRs['Username'] . " - ID#");
	  	echo($_POST['delete'][$i] . "<BR>");
	}
