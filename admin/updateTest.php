...
?>
    ...
    <tr> 
	 ...
	 // $timelimit=$_POST['TimeHours'] . ":" . $_POST['TimeMinutes'] . ":00";
	  ...
	  $testsSQL="UPDATE tests SET
	  TestName='" . addslashes($_POST['TestName']) . "',
	  PassingScore='" . addslashes($_POST['PassingScore']) . "',
	  Creator='" . addslashes($_SESSION['loggedInName']) . "',
	  Directions='" . addslashes($_POST['Directions']) . "',
	  Enabled='" . addslashes($enabled) . "',
	  RequireLogin='" . addslashes($login) . "',
	  LimitTime='" . addslashes($limit) . "',
	  TimeLimitH='" . addslashes($_POST['TimeHours']) . "',
	  TimeLimitM='" . addslashes($_POST['TimeMinutes']) . "',
	  Subject='" . addslashes($_POST['subject']) . "',
	  EmailInstructor='" . addslashes($emailInstructor) . "',
	  EmailUsers='" . addslashes($emailUsers) . "',
	  EmailTemplate='" . addslashes($_POST['emailTemplate']) . "',
	  ReportTemplate='" . addslashes($_POST['reportTemplate']) . "',
	  AutoSession='" . addslashes($autoSession) . "',
	  AllowQuit='" . addslashes($allowQuit) . "',
	  Browseable='" . addslashes($browseable) . "',
	  AltEmail='" . addslashes($altemail) . "',
	  QuestionLimit='" . addslashes($_POST['QuestionLimit']) . "',
	  MaxAttempts='" . addslashes($_POST['MaxAttempts']) . "',
	  Random='" . addslashes($random) . "'  
	  WHERE ID=" . addslashes($_POST['TestID']);
	  $result=mysqli_query( $conn,$testsSQL)
	  	or die("Invalid Query: " . $testsSQL . " - " . mysqli_error());
	  redirect_to("testManage.php");
	  // redirect_to("editTest.php?TestID=" . $_POST['TestID']);
	  ?>
